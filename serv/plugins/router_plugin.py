...
if TYPE_CHECKING:
    from serv.plugins.loader import RouteConfig, RouterConfig, PluginSpec
    from serv.plugins.importer import Importer


class RouterBuilder:
    def __init__(self, mount_path: str | None, settings: dict[str, Any], routes: "list[RouteConfig]", importer: "Importer"):
...
class RouterPlugin(Plugin):
    def __init__(self, *, plugin_spec: "PluginSpec", stand_alone: bool = False):
        ...
        self._routers: dict[str, RouterBuilder] = dict(self._setup_routers(plugin_spec.routers))

    def _setup_routers(self, routers: "list[RouterConfig]") -> Generator[tuple[str, RouterBuilder], None, None]:

...
class MultipartParser:
    ...
    """
    def __init__(self, boundary: bytes, charset: str = 'utf-8', config: Optional[Dict[str, int]] = None):
        if not boundary:
    ...
    def _on_header_field(self, data: bytes, start: int, end: int):
        if len(self._current_header_name_buffer) + (end - start) > self._max_header_name_size:
            raise MultipartParserError(f"Header name exceeds maximum size of {self._max_header_name_size} bytes.")
        self._current_header_name_buffer.extend(data[start:end])

    def _on_header_value(self, data: bytes, start: int, end: int):
        if len(self._current_header_value_buffer) + (end - start) > self._max_header_value_size:
            raise MultipartParserError(f"Header value exceeds maximum size of {self._max_header_value_size} bytes.")
    ...
    def _on_header_end(self):
        ...
        if self._current_headers_count_in_part > self._max_headers_per_part:
            raise MultipartParserError(f"Number of headers in part exceeds maximum of {self._max_headers_per_part}.")
    ...
    def _on_headers_finished(self):
        ...
        if disposition_header_value:
            ...
            if filename_bytes:
                original_filename = filename_bytes.decode(self.charset, errors='replace')
                secured_filename = secure_filename(original_filename)
                if not secured_filename and original_filename: # Filename became empty after securing
                    raise MultipartParserError(f"Unsafe filename provided: '{original_filename}'")
                if secured_filename: # Only set if filename is valid and not empty after securing
    ...
    def _on_part_data(self, data: bytes, start: int, end: int):
        if self._current_part_data_buffer:
            ...
            if self._is_file_part:
                if self._current_part_data_buffer.tell() + chunk_size > self._max_file_size:
        ...
        if self._current_part_data_buffer and self._current_part_name:
            ...
            if self._is_file_part:
                if self._total_files_count >= self._max_num_files:
                    raise MultipartParserError(f"Maximum number of files ({self._max_num_files}) exceeded.")

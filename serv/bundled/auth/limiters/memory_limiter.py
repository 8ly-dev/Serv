...
class MemoryRateLimiter(RateLimiter):
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            async with self._locks[identifier]:
                ...
                if (len(self._requests) >= self.max_tracked_identifiers and 
                    identifier not in self._requests):
                    logger.warning(f"Rate limiter at capacity ({self.max_tracked_identifiers} identifiers)")
                ...
                reset_time = datetime.fromtimestamp(
                    request_times[0] + limit_config["window_seconds"] if request_times 
                ...
                if not allowed and request_times:
                    retry_after = int(request_times[0] + limit_config["window_seconds"] - current_time)
    ...
    async def reset_rate_limit(self, identifier: str, action: str) -> None:
        ...
        try:
            if identifier in self._locks:
                async with self._locks[identifier]:
                    if identifier in self._requests and action in self._requests[identifier]:
        ...
        try:
            ...
            if identifier in self._locks:
                ...
                return RateLimitResult(
                    ...
                    reset_time=datetime.now(UTC) + timedelta(seconds=limit_config["window_seconds"]),
                    metadata={"window": limit_config["window_name"], "algorithm": "sliding_window"}
                )
                
        except Exception as e:
            logger.error(f"Error getting rate limit status for {identifier}/{action}: {e}")
            return RateLimitResult(
                allowed=True,
                limit=1000,
                remaining=999,
    ...
    async def _cleanup_if_needed(self) -> None:
        ...
        if identifiers_to_remove:
            logger.debug(f"Cleaned up {len(identifiers_to_remove)} expired rate limit entries")

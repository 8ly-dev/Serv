...
from serv.auth.utils import secure_compare, timing_protection
    ...
    async def validate_credentials(
        ...
        async with timing_protection(0.1):  # Prevent timing attacks
            try:
                ...
                return ValidationResult(
                    is_valid=False,
                    error_message="Credential validation service error"
                )
    ...
    async def validate_credential(self, credential_payload: dict[str, Any]) -> ValidationResult:
        ...
        async with timing_protection(0.1):
            try:
                ...
                return ValidationResult(
                    is_valid=False,
                    error_message="Token validation service error"
                )
            ...
            try:
                ...
                if not user_id:
                    return RefreshResult(
                        success=False,
                        error_message="No user ID in session data"
                    )

    ...
    def load_class(module_path: str) -> Type:
        ...
        try:
            if ':' not in module_path:
                raise AuthConfigError(f"Invalid module path format: {module_path}. Expected 'module:class'")
            ...
            raise AuthConfigError(f"Could not import module '{module_name}': {e}") from e
        except AttributeError as e:
            raise AuthConfigError(f"Class '{class_name}' not found in module '{module_name}': {e}") from e
...
class AuthSystemFactory:
    ...
    def create_session_storage(self, config: Dict[str, Any]) -> SessionManager:
        ...
        if 'backend' not in config:
            raise AuthConfigError("Session storage configuration missing 'backend' field")
    ...
    def create_credential_vault(self, config: Dict[str, Any]) -> CredentialVault:
        ...
        """
        if 'backend' not in config:
            raise AuthConfigError("Credential vault configuration missing 'backend' field")
...
def create_auth_system(auth_config: Dict[str, Any], container: Container | None = None) -> Dict[str, Any]:
    """

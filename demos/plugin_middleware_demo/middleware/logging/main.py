...
def request_logger_middleware(config: Optional[Dict[str, Any]] = None) -> Callable[[], AsyncIterator[None]]:
    """
    ...
    async def middleware_generator() -> AsyncIterator[None]:
        ...
        try:
            ...
            logger.info(f"Request completed: {request.method} {request.path} - {status} ({duration:.3f}s)")
        
        except Exception as exc:
            # Log the error
            duration = time.time() - start_time
            logger.error(f"Request failed: {request.method} {request.path} - {type(exc).__name__}: {exc} ({duration:.3f}s)")

...
async def test_lifespan_protocol_flow(app: App):
    ...
    try:
        await asyncio.wait_for(lifespan_task, timeout=1.0) # Wait for app to finish lifespan handling
    except asyncio.TimeoutError:
        lifespan_task.cancel()
        pytest.fail("Lifespan task did not complete in time. Check for infinite loops or deadlocks.")

    # Assertions for sent messages by the app
    assert len(sent_messages) == 2, f"Expected 2 messages sent, got {len(sent_messages)}: {sent_messages}"
...
async def test_lifespan_startup_failure_simulation(app: App):
    ...
    assert not sent_messages, "App should not have sent any messages if startup plugin error propagated out of app() call" 
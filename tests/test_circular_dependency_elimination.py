...
def test_app_implements_protocols():
    ...
    from serv.protocols import EventEmitterProtocol, AppContextProtocol
...
def test_route_uses_protocols():
    ...
    emit_sig = inspect.signature(route.emit)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                assert not alias.name.endswith('serv.app'), f"Found direct import of serv.app: {alias.name}"
        elif isinstance(node, ast.ImportFrom):
            if node.module:
                assert not node.module.endswith('serv.app'), f"Found direct import from serv.app: {node.module}"

...
from serv.auth.audit.enforcement import AuditEmitter
from serv.auth.types import User, Session, Credentials, Permission, Role, CredentialType, AuditEvent
...
class TestCredentialProvider:
    ...
    def test_credential_provider_has_required_methods(self):
        ...
        actual_methods = {name for name, method in CredentialProvider.__dict__.items() 
...
class TestSessionProvider:
    ...
    def test_session_provider_has_required_methods(self):
        ...
        actual_methods = {name for name, method in SessionProvider.__dict__.items() 
                         if getattr(method, '__isabstractmethod__', False)}
...
class TestUserProvider:
    ...
    def test_user_provider_has_required_methods(self):
        ...
        actual_methods = {name for name, method in UserProvider.__dict__.items() 
                         if getattr(method, '__isabstractmethod__', False)}
...
class TestAuthProvider:
    ...
    def test_auth_provider_has_required_methods(self):
        ...
        actual_methods = {name for name, method in AuthProvider.__dict__.items() 
                         if getattr(method, '__isabstractmethod__', False)}
...
class TestAuditProvider:
    ...
    def test_audit_provider_has_required_methods(self):
        ...
        actual_methods = {name for name, method in AuditProvider.__dict__.items() 
                         if getattr(method, '__isabstractmethod__', False)}
...
class MockCredentialProvider(CredentialProvider):
    ...
    async def create_credentials(self, user_id: str, credentials: Credentials, audit_emitter: AuditEmitter) -> None:
        self.credentials[credentials.id] = credentials
    
    async def update_credentials(self, user_id: str, old_credentials: Credentials, 
                               new_credentials: Credentials, audit_emitter: AuditEmitter) -> None:
    ...
    async def delete_credentials(self, user_id: str, credential_type: CredentialType, 
                               audit_emitter: AuditEmitter) -> None:
        to_delete = [cred_id for cred_id, cred in self.credentials.items() 
    ...
    async def get_credential_types(self, user_id: str) -> Set[CredentialType]:
        return {cred.type for cred in self.credentials.values() if cred.user_id == user_id}

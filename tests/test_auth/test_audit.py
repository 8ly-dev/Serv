...
from serv.auth.audit.enforcement import AuditJournal, AuditRequired
...
class TestAuditPipeline:
    ...
    def test_pipeline_validation_with_extra_events(self):
        ...
        pipeline = AuditPipeline(
            [AuditEventType.AUTH_ATTEMPT, AuditEventType.AUTH_SUCCESS]
        )
    ...
    def test_pipeline_validation_failure(self):
        ...
        events = [AuditEventType.AUTH_ATTEMPT, AuditEventType.AUTH_SUCCESS]
    ...
    def test_pipeline_or_operation(self):
        ...
        pipeline1 = AuditPipeline(
            [AuditEventType.AUTH_ATTEMPT, AuditEventType.AUTH_SUCCESS]
        )
        pipeline2 = AuditPipeline(
...
class TestAuditJournal:
    ...
    def test_audit_journal_creation(self):
        ...
        assert len(journal.event_data) == 0
        assert journal.sequence_id is not None

    @patch("serv.auth.audit.enforcement.AuditJournal._store_audit_event")
    def test_audit_journal_record(self, mock_store):
        """Test recording audit events."""
        journal = AuditJournal()

        data = {"user_id": "user123", "action": "login"}
        journal.record(AuditEventType.AUTH_ATTEMPT, data)

        assert len(journal.events) == 1
        assert journal.events[0] == AuditEventType.AUTH_ATTEMPT
        assert len(journal.event_data) == 1
        assert journal.event_data[0]["user_id"] == "user123"
        assert journal.event_data[0]["sequence_id"] == journal.sequence_id

        # Should call storage
        mock_store.assert_called_once()

    def test_audit_journal_sequence_tracking(self):
        """Test sequence tracking in audit journal."""
        journal = AuditJournal()

        journal.record(AuditEventType.AUTH_ATTEMPT)
        journal.record(AuditEventType.AUTH_SUCCESS)

        assert len(journal.events) == 2
        assert journal.event_data[0]["sequence_position"] == 1
        assert journal.event_data[1]["sequence_position"] == 2

        # Same sequence ID for all events
        assert (
            journal.event_data[0]["sequence_id"] == journal.event_data[1]["sequence_id"]
        )
...
class TestAuditRequired:
    ...
    def test_audit_required_pipeline_failure(self, mock_store):
        ...
        with pytest.raises(
            AuditError, match="Audit pipeline requirement not satisfied"
        ):
...
class TestProperAnnotationHandling:
    """Test proper type annotation patterns for audit journal detection."""

    def test_annotation_detection_without_decorator(self):
        """Test that annotation detection works without decorator context."""
        from serv.auth.audit.enforcement import _find_audit_journal_parameter

        def test_func(param1: str, journal: AuditJournal | None = None) -> bool:
            return True

        sig = inspect.signature(test_func)
        param_name = _find_audit_journal_parameter(sig, test_func)

        assert param_name == "journal"

    def test_annotation_detection_with_direct_type(self):
        """Test annotation detection with direct AuditJournal type."""
        from serv.auth.audit.enforcement import _find_audit_journal_parameter

        def test_func(param1: str, audit: AuditJournal) -> bool:
            return True

        sig = inspect.signature(test_func)
        param_name = _find_audit_journal_parameter(sig, test_func)

        assert param_name == "audit"

    def test_annotation_error_bubbles_up(self):
        """Test that annotation evaluation errors bubble up properly."""
        from serv.auth.audit.enforcement import _find_audit_journal_parameter

        def test_func(param1: str, journal: "UndefinedType") -> bool:
            return True

        sig = inspect.signature(test_func)
        
        # Should raise NameError when trying to resolve "UndefinedType"
        import pytest
        with pytest.raises(NameError):
            _find_audit_journal_parameter(sig, test_func)

...
async def test_multiple_middleware_order(app: App, client: AsyncClient):
    ...
    async def middleware_two(response: ResponseBuilder = dependency()) -> AsyncIterator[None]:
...
async def test_middleware_exception_before_yield(app: App, client: AsyncClient):
    ...
    assert cleanup_called # This will be true if outer_mw was started and then got an athrow
...
async def test_handler_exception_propagates_to_middleware(app: App, client: AsyncClient):
    ...
    async def observing_mw() -> AsyncIterator[None]:
        ...
        try:
            ...
            assert isinstance(error_in_athrow, HTTPNotFoundException) # Check that the correct error was passed

...
class OptionalTypesModel:
    ...
    notes: Optional[str] = "default_note"
...
@pytest.mark.asyncio
@pytest.mark.parametrize("bool_val_str, expected_bool", [
...
async def test_form_coercion_basic_type_conversion_error():
    ...
    with pytest.raises(ValueError, match=r"Error coercing value 'not-an-int' for field 'age'.*invalid literal for int\(\) with base 10: 'not-an-int'"):
        await request.form(model=BasicTypesModel)

@pytest.mark.asyncio
async def test_form_coercion_optional_types_present():
    request = await create_request_with_form_data({"description": "test desc", "count": "101"})
...
async def test_form_coercion_list_types_single_value():
    request = await create_request_with_form_data({"tags": "urgent", "scores": "10", "aliases": "solo"})
    model_instance = await request.form(model=ListTypesModel)
...
async def test_form_coercion_list_item_coercion_failure():
    ...
    with pytest.raises(ValueError, match=r"Error coercing item 'not-an-int' for field 'values'.*invalid literal for int\(\) with base 10: 'not-an-int'"):
...
async def test_form_coercion_model_instantiation_missing_required_field():
    ...
    with pytest.raises(TypeError, match=r"Failed to instantiate model AllFieldsRequiredModel.*__init__\(\) missing 1 required positional argument: 'field_b'"):
...
async def test_form_coercion_empty_form_data_custom_model_required_fields():
    request = await create_request_with_form_data({})
    with pytest.raises(TypeError, match=r"AllFieldsRequiredModel.__init__\(\) missing 2 required positional arguments: (?:\'field_a\' and \'field_b\'|\'field_b\' and \'field_a\')"):
        await request.form(model=AllFieldsRequiredModel)
...
async def test_form_coercion_invalid_content_type():
    ...
    with pytest.raises(RuntimeError, match="Cannot parse form data for Content-Type 'application/json'"):
        await request.form(model=BasicTypesModel)
...
async def test_form_coercion_unsupported_target_type_in_model():
    ...
    with pytest.raises(ValueError, match=r"Error coercing value 'wont_work' for field 'complex_field'.*Unsupported coercion for type.*NonStringConstructible.* from value 'wont_work': invalid literal for int\(\) with base 10: 'wont_work'"):
...
async def test_form_coercion_list_with_non_optional_int_empty_item_fails():
    request = await create_request_with_form_data({"items": ["1", "", "3"]})
    with pytest.raises(ValueError, match=r"Error coercing item '' for field 'items'.*invalid literal for int\(\) with base 10: ''"):

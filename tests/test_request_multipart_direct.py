...
async def handle_direct_single_file(request: Request = dependency(), response_builder: ResponseBuilder = dependency()):
    ...
    if not file_upload_list or not isinstance(file_upload_list, list) or not len(file_upload_list) == 1:
...
async def test_direct_multipart_single_file(app: App, client: AsyncClient):
    app.add_plugin(DirectHandlerPlugin("/direct_upload_single", handle_direct_single_file, methods=["POST"]))
...
async def handle_direct_multiple_files(request: Request = dependency(), response_builder: ResponseBuilder = dependency()):
    form_data = await request.form()
...
async def test_direct_multipart_multiple_files(app: App, client: AsyncClient):
    app.add_plugin(DirectHandlerPlugin("/direct_upload_multi", handle_direct_multiple_files, methods=["POST"]))
    
    files_payload = {
        "mandatory_file": ("main.dat", b"Mandatory content", "application/octet-stream"),
    ...
    response = await client.post("/direct_upload_multi", files=files_payload, data=data_payload)

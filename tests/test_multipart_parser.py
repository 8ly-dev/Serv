...
from serv.multipart_parser import MultipartParser, ParsedFileUpload, MultipartParserError, DEFAULT_MULTIPART_CONFIG
...
def create_file_data_part(
    ...
    headers_str = f'Content-Disposition: form-data; name="{name}"; filename="{filename}"\r\n'
...
async def mock_receive_callable(body_chunks: list[bytes]) -> Callable[[], Awaitable[dict[str, Any]]]:
    idx = 0
...
async def test_parse_simple_file(default_parser: MultipartParser):
    ...
    parts = [
        create_file_data_part("file1", "test.txt", file_content, "text/plain", boundary),
    ]
    body = create_multipart_body(parts, boundary)
    receive = await mock_receive_callable([body])
...
async def test_parse_multiple_fields_and_files(default_parser: MultipartParser):
    ...
    parts = [
        ...
        create_file_data_part("upload1", "first.dat", file_content1, "application/octet-stream", boundary),
        create_form_data_part("fieldB", "valB", boundary),
        create_file_data_part("upload2", "second.dat", file_content2, "image/png", boundary),
    ]
    body = create_multipart_body(parts, boundary)
    receive = await mock_receive_callable([body])
...
async def test_max_file_size_exceeded(custom_config_parser_factory):
    ...
    parts = [create_file_data_part("bigfile", "large.txt", file_content, "text/plain", boundary)]
    body = create_multipart_body(parts, boundary)
    receive = await mock_receive_callable([body])
...
async def test_max_num_files_exceeded(custom_config_parser_factory):
    ...
    with pytest.raises(MultipartParserError, match="Maximum number of files .* exceeded"):
        await parser.parse(receive)
...
async def test_max_total_body_size_exceeded(custom_config_parser_factory):
    ...
    part2 = create_file_data_part("file1", "f.txt", b"c" * file_content_len, "text/plain", boundary)
    ...
    with pytest.raises(MultipartParserError, match="Total request body size exceeds maximum"):
        await parser.parse(receive)
...
async def test_max_header_name_size_exceeded(custom_config_parser_factory):
    ...
    parts = [
        create_file_data_part("file1", "f.txt", b"c", "text/plain", boundary, 
...
async def test_max_headers_per_part_exceeded(custom_config_parser_factory):
    ...
    with pytest.raises(MultipartParserError, match="Number of headers in part exceeds maximum"):
        await parser.parse(receive)

# --- Filename Security Tests ---

@pytest.mark.asyncio
@pytest.mark.parametrize("unsafe_filename, description", [
...
async def test_unsafe_filenames_rejected_or_sanitized(default_parser: MultipartParser, unsafe_filename: str, description: str):
    boundary = "testboundary"
    parts = [create_file_data_part("upload", unsafe_filename, b"content", "text/plain", boundary)]
    body = create_multipart_body(parts, boundary)
    receive = await mock_receive_callable([body])
    ...
    if not expected_filename and unsafe_filename:
        with pytest.raises(MultipartParserError, match=f"Unsafe filename provided: '{unsafe_filename}'"):
            await default_parser.parse(receive)
    else:
        result = await default_parser.parse(receive)
        if not expected_filename:
            if "upload" in result and result["upload"]:
                 assert not isinstance(result["upload"][0], ParsedFileUpload), "Should not be a file upload if filename became empty"
                 assert isinstance(result["upload"][0], str), "Should be a string field if filename was invalid"
            assert not default_parser.files.get("upload"), "File should not appear in parser.files if filename was invalid"

        else:
            assert "upload" in result, f"Key 'upload' missing for {description}"
            assert len(result["upload"]) == 1, f"Incorrect number of files for {description}"
            file_upload: ParsedFileUpload = result["upload"][0]
            assert file_upload["filename"] == expected_filename, f"Filename sanitization failed for: {description}"

@pytest.mark.asyncio
async def test_empty_filename_string_not_treated_as_file(default_parser: MultipartParser):
    boundary = "testboundary"
    parts = [create_file_data_part("upload_empty_fn", "", b"content", "text/plain", boundary)]
    body = create_multipart_body(parts, boundary)
    receive = await mock_receive_callable([body])
...
@pytest.mark.asyncio
async def test_no_filename_directive_in_content_disposition(default_parser: MultipartParser):
    boundary = "testboundary"
    part_data = (
        f'--{boundary}\r\n'
...
@pytest.mark.asyncio
async def test_malformed_content_disposition_rejected(default_parser: MultipartParser, mocker):
    ...
    part_data = create_file_data_part("upload", "test.txt", b"content", "text/plain", boundary)
...
@pytest.mark.asyncio
async def test_non_latin1_chars_in_header_value_disposition_filename(default_parser: MultipartParser):
    boundary = "testboundary"
    non_latin1_filename_value = "файл.txt"

    header_value_with_unicode_filename = f'form-data; name="upload"; filename="{non_latin1_filename_value}"'
    
    default_parser._current_part_headers = {'content-disposition': header_value_with_unicode_filename}
    ...
    with pytest.raises(UnicodeEncodeError, match="'latin-1' codec can't encode character"):
...
async def test_chunked_body_processing(default_parser: MultipartParser):
    ...
    parts = [
        ...
        create_file_data_part("file1_chunked", "chunked_test_long.txt", file_content, "text/plain", boundary),
    ]
    body = create_multipart_body(parts, boundary)
    ...
    assert field_list is None or not any(isinstance(item, str) and item.startswith("this is a test file") for item in field_list) 